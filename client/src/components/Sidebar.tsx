...
import { Home, Laptop, Users, LogOut } from "lucide-react";
    ...
    <aside className="hidden md:flex md:flex-col fixed top-0 left-0 h-screen w-60 bg-gradient-to-b from-primary to-primary-dark text-sm py-8 px-4 z-40">
      ...
      <nav className="flex flex-col gap-2">
        <NavLink to="/" className={navItemClass}>
          <Home className="text-base" /> Dashboard
        </NavLink>
        <NavLink to="/echipamente" className={navItemClass}>
          <Laptop className="text-base" /> Echipamente
        </NavLink>
        <NavLink to="/colegi" className={navItemClass}>
          <Users className="text-base" /> Colegi
        </NavLink>
      </nav>
